...
export const IntelligenceCard: React.FC<IntelligenceCardProps> = ({
  ...
  const [sortBy, setSortBy] = useState<'confidence' | 'priority' | 'size'>(
    'confidence'
  );
  ...
  const totalBatchDetections = batchScans.reduce(
    (acc, s) => acc + s.detections.length,
    0
  );
  ...
  return (
    <div className="bg-white/90 backdrop-blur-md rounded-3xl p-4 sm:p-6 border border-slate-200/80 shadow-md flex flex-col justify-between h-full min-w-0">
      <div className="min-w-0">
        {/* Header */}
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-4">
          <div className="flex items-start gap-3 min-w-0">
            <div className="w-10 h-10 rounded-xl bg-slate-100 flex items-center justify-center text-slate-700 shadow-2xs border border-slate-200/60 shrink-0">
          ...
          <div className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-amber-50 border border-amber-300 text-amber-900 text-xs font-bold shadow-2xs max-w-full">
            <Target className="w-3.5 h-3.5 text-amber-700 shrink-0" />

            <span className="truncate">
              {batchScans.length > 1
                ? `Frame: ${detections.length} Target${
                    detections.length === 1 ? '' : 's'
                  } | Batch: ${totalBatchDetections} Total`
                : scan
                ? `${detections.length} ${
                    detections.length === 1 ? 'Target Found' : 'Targets Found'
                  }`
                : 'Ready'}
            </span>
          </div>
        ...
        <p className="text-xs text-slate-500 mb-5 leading-relaxed">
          Detected objects, classifications and actionable insights from your
          sonar data.
        </p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
          {/* Images */}
          <div className="bg-white rounded-2xl p-3.5 border border-slate-200/80 shadow-xs flex items-center justify-between min-w-0">
          ...
          <div className="bg-white rounded-2xl p-3.5 border border-slate-200/80 shadow-xs flex items-center justify-between min-w-0">
            <div className="flex items-center gap-2.5 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <div className="text-lg font-bold text-slate-800 leading-tight">
                  {batchScans.length > 0
                    ? totalBatchDetections
                    : detections.length}
                </div>

                <div className="text-[11px] text-slate-500">
                  {batchScans.length > 1
                    ? `Found in ${batchScans.length} images`
                    : 'Objects found'}
                </div>
              </div>
            </div>
        ...
        {scan && sq && (
          <div className="bg-slate-50/90 rounded-2xl p-3 sm:p-4 border border-slate-200/90 shadow-xs mb-5 min-w-0">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 mb-3">
              <div className="flex items-center gap-2">
                <span className="w-1.5 h-4 bg-amber-600 rounded-full shrink-0" />
              ...
              {ea && (
                ...
                >
                  ...
                  <span>
                    Evidence: {ea.evidenceScore ?? ea.evidence_score}/100 (
                    {ea.reliability || ea.reliability_level})
                  </span>
                </div>
              )}
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 mb-3">
              {/* Speckle Noise */}
              <div className="bg-white rounded-xl p-2.5 border border-slate-200/80 shadow-2xs min-w-0">
                <div className="flex items-center justify-between mb-1 gap-1">
                  <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wide flex items-center gap-1 min-w-0">
                    <Waves className="w-3 h-3 text-amber-700 shrink-0" />
                    <span className="truncate">Speckle Noise</span>
                  </span>
                ...
                <div className="text-xs font-bold text-slate-800">
                  Cv:{' '}
                  {sp?.speckleIndex !== undefined
                    ? Number(sp.speckleIndex).toFixed(2)
                    : sp?.speckle_index !== undefined
                    ? Number(sp.speckle_index).toFixed(2)
                    : 'N/A'}
                </div>
                ...
                >
                  ENL: {sp?.enl !== undefined ? Number(sp.enl).toFixed(1) : 'N/A'}{' '}
                  looks
                </div>
              </div>
              ...
              <div className="bg-white rounded-xl p-2.5 border border-slate-200/80 shadow-2xs min-w-0">
                <div className="flex items-center justify-between mb-1 gap-1">
                  <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wide flex items-center gap-1 min-w-0">
                    ...
                    <span className="truncate">2D Sharpness</span>
                  </span>
                  ...
                  <span
                    className={`text-[9px] font-bold px-1.5 py-0.2 rounded-md shrink-0 ${
                      (rq?.qualityRating || rq?.quality_rating) === 'OPTIMAL'
                        ? 'bg-stone-100 text-stone-800'
                        : (rq?.qualityRating || rq?.quality_rating) ===
                          'ACCEPTABLE'
                        ? 'bg-amber-100 text-amber-900'
                ...
                <div className="text-[10px] text-slate-500 truncate">
                  Contrast:{' '}
                  {rq?.contrastScore ?? rq?.contrast_score ?? 'N/A'}
                  /100
                </div>
              </div>
              ...
              <div className="bg-white rounded-xl p-2.5 border border-slate-200/80 shadow-2xs min-w-0">
                <div className="flex items-center justify-between mb-1 gap-1">
                  <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wide flex items-center gap-1 min-w-0">
                    ...
                    <span className="truncate">Shadow Relief</span>
                  </span>

                  <span
                    className={`text-[9px] font-bold px-1.5 py-0.2 rounded-md shrink-0 ${
                      (ash?.evidenceStrength ||
                        ash?.evidence_strength) === 'STRONG'
                        ? 'bg-stone-100 text-stone-800'
                        : (ash?.evidenceStrength ||
                            ash?.evidence_strength) === 'MODERATE'
                        ? 'bg-amber-100 text-amber-900'
                        : 'bg-slate-100 text-slate-700'
                    }`}
                  >
                    {ash?.evidenceStrength ||
                      ash?.evidence_strength ||
                      'UNAVAILABLE'}
                  </span>
                </div>
              ...
              <div className="bg-white rounded-xl p-2.5 border border-slate-200/80 shadow-2xs min-w-0">
                <div className="flex items-center justify-between mb-1 gap-1">
                  <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wide flex items-center gap-1 min-w-0">
                    <Activity className="w-3 h-3 text-amber-700 shrink-0" />
                    <span className="truncate">Scanline Ping</span>
                  </span>

                  <span
                    className={`text-[9px] font-bold px-1.5 py-0.2 rounded-md shrink-0 ${
                      (doArt?.artifactSeverity ||
                        doArt?.artifact_severity) === 'NONE'
                        ? 'bg-stone-100 text-stone-800'
                        : (doArt?.artifactSeverity ||
                            doArt?.artifact_severity) === 'LOW'
                        ? 'bg-amber-100 text-amber-900'
                        : 'bg-amber-200 text-amber-950'
                    }`}
                  >
                    {doArt?.artifactSeverity ||
                      doArt?.artifact_severity ||
                      'UNAVAILABLE'}
                  </span>
                </div>
                ...
                <div className="text-[10px] text-slate-500 truncate">
                  {doArt?.dropoutPercentage ??
                    doArt?.dropout_percentage ??
                    0.0}
                  % missing pings
                </div>
              </div>
            ...
            <div className="flex items-start gap-2 bg-slate-100/80 rounded-xl p-2 text-[11px] text-slate-600 border border-slate-200/60">
              ...
              <span className="leading-snug">
                <strong>Hydrographic Notice:</strong> 2D image analysis
                measures pixel-level features and scanline integrity only. Does{' '}
                <strong>NOT</strong> claim to measure physical towfish heave,
                pitch, or roll without external IMU / AHRS sensor telemetry.
              </span>
            </div>
          </div>
        )}
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 mb-3">
          <div className="flex items-center gap-2 text-sm font-bold text-slate-800 min-w-0">
            <span className="w-1 h-3.5 bg-slate-800 rounded-full shrink-0" />

            <span className="truncate">
              Detection Results {scan && `— ${scan.filename}`}
            </span>
          </div>
        ...
        {scan && sortedDetections.length > 0 ? (
          <div className="space-y-4">
            {sortedDetections.map((det, index) => {
              ...
              const detShadow =
                det.shadowMetrics || (det as any).acoustic_shadow;

              const detEvScore =
                det.evidenceScore ?? (det as any).evidence_score;

              const detReliability =
                det.reliability ?? (det as any).reliability;

              const detReviewReq =
                det.reviewRequired ?? (det as any).review_required;

              const detReasons =
                det.reviewReasons ||
                (det as any).review_reasons ||
                [];

              return (
                <div
                  key={det.id}
                  className="bg-white rounded-2xl p-4 border border-slate-200/90 shadow-sm transition-all hover:shadow-md animate-in fade-in-50 min-w-0"
                >
                  {/* Detection Header */}
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-3">
                    <div className="flex items-center gap-2.5 min-w-0">
                      <span className="bg-stone-100 text-stone-800 font-bold text-xs px-2 py-0.5 rounded-md shrink-0">
                        #{index + 1}
                      </span>
                      ...
                      {detShadow && (
                        <span
                          className={`text-[10px] font-bold px-2 py-0.5 rounded-md ${
                            (detShadow.evidenceStrength ||
                              detShadow.evidence_strength) === 'STRONG'
                              ? 'bg-purple-100 text-purple-800'
                              : (detShadow.evidenceStrength ||
                                  detShadow.evidence_strength) === 'MODERATE'
                              ? 'bg-amber-100 text-amber-900'
                              : 'bg-slate-100 text-slate-600'
                          }`}
                        >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="flex flex-col justify-between space-y-2.5 min-w-0">
                      <div className="grid grid-cols-2 gap-2">
                        ...
                        <div className="bg-white/70 backdrop-blur-sm rounded-xl p-2.5 border border-slate-200/80 shadow-xs flex items-center gap-2.5 min-w-0">
                          ...
                          <div className="min-w-0">
                            ...
                            <div className="text-xs font-bold text-slate-800 tracking-tight leading-tight mt-0.5">
                              {det.pixelDimensions.width} ×{' '}
                              {det.pixelDimensions.height}
                            </div>
                      ...
                      <div className="flex items-start gap-2 text-xs bg-slate-50/70 p-2 rounded-xl border border-slate-100">
                        <span className="text-stone-600 mt-0.5 shrink-0">
                          🌿
                        </span>
                      ...
                      <div className="flex items-start gap-2 text-xs bg-amber-50/50 p-2 rounded-xl border border-amber-100/70">
                        ...
                        <div className="min-w-0">
                          ...
                          <span className="text-amber-800">
                            {det.actionRecommended}
                          </span>
                        </div>
                      </div>

                      {/* Bounding Box */}
                      <div className="flex items-center gap-2 text-[11px] text-slate-500 font-mono bg-slate-50 p-1.5 rounded-lg border border-slate-100 min-w-0">
                      ...
                      {/* Verify / Reject / Bookmark */}
                      <div className="flex items-center gap-2 pt-1 border-t border-slate-100">
                        <button
                          onClick={() => handleVerify(det.id)}
                          className={`flex-1 min-w-0 py-2 px-3 rounded-xl font-bold text-xs flex items-center justify-center gap-1.5 transition-all cursor-pointer shadow-xs ${
                        ...
                        >
                          ...
                          <span className="w-2 h-2 rounded-full bg-stone-600 ml-1 shrink-0" />
                        </button>
                        ...
                        >
                          ...
                          <span className="w-2 h-2 rounded-full bg-stone-400 ml-1 shrink-0" />
                        </button>
            ...
            <div className="flex justify-end pt-1">
              ...
              >
                ...
                <span>
                  Download Executive Survey Report (PDF)
                </span>
              </button>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl p-4 sm:p-6 border border-slate-200/90 shadow-sm space-y-4">
            <div className="flex items-start gap-2.5 text-stone-800 bg-stone-100 px-3 py-2 rounded-xl border border-stone-300">
              <CheckCircle className="w-5 h-5 text-stone-700 shrink-0" />

              <div className="text-xs font-semibold">
                Clean Seabed: No debris or hazards detected by YOLO11n
                (best.pt) in this frame.
              </div>
            </div>
          ...
          <div className="min-h-64 h-64 flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-slate-200 p-6 text-center bg-slate-50/50">
            <div className="w-12 h-12 rounded-full bg-slate-100 text-slate-400 flex items-center justify-center mb-3">
              <Brain className="w-6 h-6 text-slate-400" />
            </div>

            <div className="text-sm font-bold text-slate-700">
              Awaiting Sonar Imagery
            </div>

            <div className="text-xs text-slate-500 max-w-sm mt-1 leading-relaxed">
              Drag & drop a sonar image or select multiple survey frames on
              the left and click{' '}
              <span className="font-semibold text-amber-700">
                "Detect Objects"
              </span>{' '}
              to run live inference with your trained{' '}
